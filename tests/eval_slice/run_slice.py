...
# Date-aware ingestion shim shared with scripts/run_full_*.py runners.
from tests.eval_slice.dated_ingestion import (
    ingest_locomo_with_dates,
    ingest_longmemeval_with_dates,
...
def _run_locomo(curated: list[dict], backend_kwargs: dict) -> tuple[dict, float]:
    ...
    for question in filtered:
        ...
        store.reset()
        ingest_locomo_with_dates(
            store, question, date_map.get(question.conversation_id, []),
        )
...
def _run_longmemeval(curated: list[dict], backend_kwargs: dict) -> tuple[dict, float]:
    ...
    for question in filtered:
        store = MnemoriaBenchmarkAdapter(**backend_kwargs)
        store.reset()
        ingest_longmemeval_with_dates(store, question)
